...
{
  "$end", "error", "$undefined", "NOTE", "CHORD_TYPE", "$accept",
...
#line 41 "music.y"
    {
    if (!is_valid_chord((yyvsp[(1) - (2)].note), (yyvsp[(2) - (2)].chord_type))) {
        yyerror("Invalid chord in the scale");
    } else {
        play_sound((yyvsp[(1) - (2)].note), (yyvsp[(2) - (2)].chord_type));
    }
;}
    break;
